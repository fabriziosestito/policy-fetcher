...
mod tests {
    ...
    fn test_encode_path() -> Result<()> {
        let expected_path = PathBuf::from("/")
            .join("registry")
            .join(BASE64_ENGINE.encode("ghcr.io"))
            .join(BASE64_ENGINE.encode("some"))
            .join(BASE64_ENGINE.encode("path"))
            .join(BASE64_ENGINE.encode("to"))
            .join(BASE64_ENGINE.encode("wasm-module.wasm:1.0.0"));
    ...
    fn test_decode_path() -> Result<()> {
        assert_eq!(
            ...
            decode_path(
                PathBuf::from("/")
                    .join("registry")
                    .join(BASE64_ENGINE.encode("example.com:1234"))
                    .join(BASE64_ENGINE.encode("some"))
                    .join(BASE64_ENGINE.encode("path"))
                    .join(BASE64_ENGINE.encode("to"))
                    .join(BASE64_ENGINE.encode("wasm-module.wasm:1.0.0"))
